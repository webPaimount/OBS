...
{
	return !strncmp(stream->ff_data.config.url, RIST_PROTO, sizeof(RIST_PROTO) - 1);
}

static bool is_srt(struct ffmpeg_output *stream)
{
	return !strncmp(stream->ff_data.config.url, SRT_PROTO, sizeof(SRT_PROTO) - 1);
}
...
static inline int allocate_custom_aviocontext(struct ffmpeg_output *stream, bool is_rist)
{
	...
	if (is_rist)
		s = avio_alloc_context(buffer, buffer_size, AVIO_FLAG_WRITE, h, NULL,
