...
#define use_old_nvenc false
	...
	if (use_old_nvenc ||
	    (obs_data_has_user_value(settings, "preset") && !obs_data_has_user_value(settings, "preset2"))) {
...
#define use_old_nvenc false
	...
	p = obs_properties_add_list(props, use_old_nvenc ? "preset" : "preset2", obs_module_text("Preset"),
...
#undef add_profile
	...
	if (!ffmpeg) {
		p = obs_properties_add_bool(props, "lookahead", obs_module_text("NVENC.LookAhead"));
		obs_property_set_long_description(p, obs_module_text("NVENC.LookAhead.ToolTip"));
