...
static void receive_audio(void *param, size_t mix_idx, struct audio_data *frame)
{
	...
	for (size_t i = 0; i < data->audio_planes; i++)
		circlebuf_push_back(&data->excess_frames[track_order][i], in.data[i], in.frames * data->audio_size);

	while (data->excess_frames[track_order][0].size >= frame_size_bytes) {
		for (size_t i = 0; i < data->audio_planes; i++)
			circlebuf_pop_front(&data->excess_frames[track_order][i], data->samples[track_order][i],
					    frame_size_bytes);
