...
static void add_video_encoder_params(struct ffmpeg_muxer *stream, struct dstr *cmd, obs_encoder_t *vencoder)
{
	...
	dstr_catf(cmd, "%s %d %d %d %d %d %d %d %d %d %d %d %d ", obs_encoder_get_codec(vencoder), bitrate,
		  obs_output_get_width(stream->output), obs_output_get_height(stream->output), (int)pri, (int)trc,
		  (int)spc, (int)range, (int)determine_chroma_location(obs_to_ffmpeg_video_format(info->format), spc),
		  max_luminance, (int)info->fps_num, (int)info->fps_den, (int)codec_tag);
}
...
static void add_audio_encoder_params(struct dstr *cmd, obs_encoder_t *aencoder)
{
	...
	dstr_catf(cmd, "\"%s\" %d %d %d %d ", name.array, bitrate, (int)obs_encoder_get_sample_rate(aencoder),
		  (int)obs_encoder_get_frame_size(aencoder), (int)audio_output_get_channels(audio));
...
static void build_command_line(struct ffmpeg_muxer *stream, struct dstr *cmd, const char *path)
{
...
static inline void purge(struct ffmpeg_muxer *stream)
{
	if (purge_front(stream)) {
		...
		for (;;) {
			if (!stream->packets.size)
				return;
			circlebuf_peek_front(&stream->packets, &pkt, sizeof(pkt));
