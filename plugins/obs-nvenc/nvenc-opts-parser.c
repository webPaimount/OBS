...
/* NVIDIA likes their bitfields, and unfortunately it is not possible to get a
 * pointer or offset for struct members that are bitfields. So the only way to
 * solve this is to use macros. */

#define APPLY_BIT_OPT(opt_name, bits)                                     \
	if (strcmp(opt->name, #opt_name) == 0) {                          \
		uint32_t val = strtoul(opt->value, NULL, 10) &            \
			       ((1 << (bits)) - 1);                       \
		blog(LOG_DEBUG, "[obs-nvenc] Changing: \"%s\": %u -> %u", \
		     #opt_name, nv_conf->opt_name, val);                  \
		nv_conf->opt_name = val;                                  \
		return true;                                              \
	}
