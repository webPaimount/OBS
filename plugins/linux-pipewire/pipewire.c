...
static bool lookup_format_info_from_spa_format(uint32_t spa_format, struct format_data *out_format_data)
{
...
static inline struct spa_pod *build_format(struct spa_pod_builder *b, struct obs_video_info *ovi, uint32_t format,
					   uint64_t *modifiers, size_t modifier_count)
{
	...
	spa_pod_builder_add(b, SPA_FORMAT_VIDEO_size,
			    ...
			    SPA_POD_CHOICE_RANGE_Fraction(&SPA_FRACTION(ovi->fps_num, ovi->fps_den),
				...
				const struct spa_pod ***param_list, uint32_t *n_params)
{
	...
	for (size_t i = 0; i < obs_pw_stream->format_info.num; i++) {
		...
		params[params_count++] = build_format(pod_builder, &obs_pw_stream->video_info,
...
static void init_format_info_async(obs_pipewire_stream *obs_pw_stream)
{
	...
	for (size_t i = 0; i < N_SUPPORTED_FORMATS_ASYNC; i++) {
		...
		if (!lookup_format_info_from_spa_format(supported_formats_async[i], &format_data))
			continue;
...
static void init_format_info_sync(obs_pipewire_stream *obs_pw_stream)
{
	...
	for (size_t i = 0; i < N_SUPPORTED_FORMATS_SYNC; i++) {
		struct format_data format_data;
		struct format_info *info;
		if (!lookup_format_info_from_spa_format(supported_formats_sync[i], &format_data) ||
		...
		if (!capabilities_queried || !drm_format_available(format_data.drm_format, drm_formats, n_drm_formats))
			continue;

		size_t n_modifiers;
		uint64_t *modifiers = NULL;
		if (gs_query_dmabuf_modifiers_for_format(format_data.drm_format, &modifiers, &n_modifiers)) {
...
static bool prepare_obs_frame(obs_pipewire_stream *obs_pw_stream, struct obs_source_frame *frame)
{
	...
	if (!lookup_format_info_from_spa_format(obs_pw_stream->format.info.raw.format, &format_data) ||
...
static void process_video_sync(obs_pipewire_stream *obs_pw_stream)
{
	...
	if (buffer->datas[0].type == SPA_DATA_DmaBuf) {
		...
		obs_pw_stream->texture = gs_texture_create_from_dmabuf(obs_pw_stream->format.info.raw.size.width,
								       ...
								       format_data.drm_format, GS_BGRX, planes, fds,
								       strides, offsets,
...
read_metadata:
	...
	if (obs_pw_stream->cursor.visible && obs_pw_stream->cursor.valid) {
		...
		if (bitmap && bitmap->size.width > 0 && bitmap->size.height > 0 &&
		    lookup_format_info_from_spa_format(bitmap->format, &format_data) &&
		    format_data.gs_format != GS_UNKNOWN) {
			...
			obs_pw_stream->cursor.texture =
				gs_texture_create(obs_pw_stream->cursor.width, obs_pw_stream->cursor.height,
						  format_data.gs_format, 1, &bitmap_data, GS_DYNAMIC);
...
static void on_core_error_cb(void *user_data, uint32_t id, int seq, int res, const char *message)
{
	...
	blog(LOG_ERROR, "[pipewire] Error id:%u seq:%d res:%d (%s): %s", id, seq, res, g_strerror(res), message);
...
obs_pipewire *obs_pipewire_create(int pipewire_fd)
{
	...
	obs_pw->server_version_sync = pw_core_sync(obs_pw->core, PW_ID_CORE, obs_pw->server_version_sync);
	pw_thread_loop_wait(obs_pw->thread_loop);
	pw_thread_loop_unlock(obs_pw->thread_loop);
						 ...
						 const char *stream_name, struct pw_properties *stream_properties)
{
	...
	/* Stream */
	obs_pw_stream->stream = pw_stream_new(obs_pw->core, stream_name, stream_properties);
