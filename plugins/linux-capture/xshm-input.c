...
static bool xshm_server_changed(obs_properties_t *props, obs_property_t *p, obs_data_t *settings)
{
	...
	for (int_fast32_t i = 0; i < count; ++i) {
		...
		dstr_printf(&screen_info,
			    "Screen %s (%" PRIuFAST32 "x%" PRIuFAST32 " @ %" PRIuFAST32 ",%" PRIuFAST32 ")", name, w, h,
			    x, y);
	...
	if (old_screen + 1 > count) {
		dstr_printf(&screen_info, "Screen %" PRIuFAST32 " (not found)", old_screen);
...
static obs_properties_t *xshm_properties(void *vptr)
{
	...
	obs_properties_add_list(props, "screen", obs_module_text("Screen"), OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);
