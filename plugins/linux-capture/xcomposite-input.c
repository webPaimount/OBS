...
xcb_window_t xcomp_find_window(xcb_connection_t *conn, Display *disp, const char *str)
{
	...
	if (!str || strlen(str) == 0) {
		if (tlw.num > 0)
			ret = *(xcb_window_t *)darray_item(sizeof(xcb_window_t), &tlw.da, 0);
		goto cleanup1;
	}
...
static obs_properties_t *xcompcap_props(void *unused)
{
	...
	qsort(window_strings.array, window_strings.num, sizeof(struct WindowInfo), cmp_wi);

	for (size_t i = 0; i < window_strings.num; i++) {
		struct WindowInfo *s =
			(struct WindowInfo *)darray_item(sizeof(struct WindowInfo), &window_strings.da, i);

		obs_property_list_add_string(prop, s->name.array, s->desc.array);
