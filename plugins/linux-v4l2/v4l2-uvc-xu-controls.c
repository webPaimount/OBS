 ...
 */
bool v4l2_compare_vendor_product(const char *device_path,
				 const char *vendor_product_to_compare)
{
	...
	snprintf(vendor_path, PATH_MAX,
		 "/sys/class/video4linux/%s/../../../idVendor", device_name);
	snprintf(product_path, PATH_MAX,
		 "/sys/class/video4linux/%s/../../../idProduct", device_name);
	...
	if (access(vendor_path, F_OK) == 0 && access(product_path, F_OK) == 0) {
		...
		return vendor_product ==
		       strstr(vendor_product, vendor_product_to_compare);
	}
...
int v4l2_find_unit_id_in_sysfs(const char *device_path, char *guid)
{
	...
	snprintf(descriptors_path, PATH_MAX,
		 "/sys/class/video4linux/%s/../../../descriptors", device_name);

	char buf[0x10000];
	if (access(descriptors_path, F_OK) == 0 &&
	    access(descriptors_path, F_OK) == 0) {
...
static bool v4l2_xu_control_changed(void *data, obs_properties_t *props,
				    obs_property_t *prop, obs_data_t *settings)
{
	...
	for (int query_no = 0; query_no < position->queries_cnt; query_no++) {
		struct uvc_xu_control_query *xu_query =
			&position->queries[query_no];
		if (ioctl(dev, UVCIOC_CTRL_QUERY, xu_query) < 0) {
			const char *errstr = strerror(errno);
			blog(LOG_ERROR, "Error changing XU setting: %s",
			     errstr);
		}
	}
...
int_fast32_t v4l2_xu_update_controls(char unit_id, struct menu_item *menu,
				     size_t menu_cnt, obs_properties_t *props,
				     obs_data_t *settings)
{
	obs_property_t *prop;
	...
	for (size_t menu_pos = 0; menu_pos < menu_cnt; menu_pos++) {
		...
		prop = obs_properties_add_list(props, menu_item->id,
					       menu_item->label,
					       OBS_COMBO_TYPE_LIST,
					       OBS_COMBO_FORMAT_INT);

		for (int item_pos = 0; item_pos < menu_item->positions_cnt;
		     item_pos++) {
			struct menu_item_pos *menu_item_pos =
				&menu_item->positions[item_pos];
			obs_property_list_add_int(prop, menu_item_pos->label,
						  item_pos);
			for (int query_pos = 0;
			     query_pos < menu_item_pos->queries_cnt;
			     query_pos++) {
				struct uvc_xu_control_query *query =
					&menu_item_pos->queries[query_pos];
				query->unit = unit_id;
			}
		}

		obs_property_set_modified_callback2(
			prop, v4l2_xu_control_changed, menu_item);
	}

	return 0;
}

int_fast32_t v4l2_update_uvc_xu_controls(const char *device_path,
					 ...
					 obs_data_t *settings)
{
	...
	if (v4l2_compare_vendor_product(device_path,
					LOGITECH_BRIO_VENDOR_PRODUCT)) {
		unit_id = v4l2_find_unit_id_in_sysfs(device_path,
						     LOGITECH_BRIO_GUID);
		if (-1 == unit_id) {
			blog(LOG_WARNING,
			     "Error getting unit_id for device: %s",
			     device_path);
			return -1;
		}
		return v4l2_xu_update_controls(unit_id, BRIO_MENU,
					       sizeof(BRIO_MENU) /
	...
	if (v4l2_compare_vendor_product(device_path, KIYO_PRO_VENDOR_PRODUCT)) {
		unit_id =
			v4l2_find_unit_id_in_sysfs(device_path, KIYO_PRO_GUID);
		if (-1 == unit_id) {
			blog(LOG_WARNING,
			     "Error getting unit_id for device: %s",
			     device_path);
		...
		return v4l2_xu_update_controls(unit_id, KIYO_PRO_MENU,
					       sizeof(KIYO_PRO_MENU) /
						       sizeof(struct menu_item),
					       props, settings);
	}
