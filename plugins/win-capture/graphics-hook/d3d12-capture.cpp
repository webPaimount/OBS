...
static bool create_d3d12_tex(bb_info &bb)
{
	...
	for (UINT i = 0; i < bb.count; i++) {
		hr = data.device11on12->CreateWrappedResource(bb.backbuffer[i], &rf11, D3D12_RESOURCE_STATE_PRESENT,
...
static inline bool d3d12_init_format(IDXGISwapChain *swap, HWND &window, bb_info &bb)
{
	...
	bb.count = desc.SwapEffect == DXGI_SWAP_EFFECT_DISCARD ? 1 : desc.BufferCount;
