...
OBSAdvAudioCtrl::OBSAdvAudioCtrl(QGridLayout *, obs_source_t *source_) : source(source_)
{
	...
	deactivateSignal.Connect(handler, "deactivate", OBSSourceDeactivated, this);
	...
	if (obs_audio_monitoring_available())
		monitoringTypeSignal.Connect(handler, "audio_monitoring", OBSSourceMonitoringTypeChanged, this);
	mixersSignal.Connect(handler, "audio_mixers", OBSSourceMixersChanged, this);
	balChangedSignal.Connect(handler, "audio_balance", OBSSourceBalanceChanged, this);
