...
void SimpleOutput::Update()
{
	...
	obs_data_set_string(videoSettings, (strcmp(presetType, "NVENCPreset2") == 0) ? "preset2" : "preset", preset);
...
bool SimpleOutput::SetupStreaming(obs_service_t *service)
{
	...
	if (outputType != type) {
		...
		streamDelayStarting.Connect(obs_output_get_signal_handler(streamOutput), "starting", OBSStreamStarting,
					    this);
		streamStopping.Connect(obs_output_get_signal_handler(streamOutput), "stopping", OBSStreamStopping,
				       this);
...
AdvancedOutput::AdvancedOutput(OBSBasic *main_) : BasicOutputHandler(main_)
{
	...
	streamAudioEnc =
		obs_audio_encoder_create(streamAudioEncoder, "adv_stream_audio", nullptr, streamTrack, nullptr);
	if (!streamAudioEnc)
