...
{
	const char *config_branch = config_get_string(GetGlobalConfig(), "General", "UpdateBranch");
	if (!config_branch)
		return true;
...
try {
	...
	if (!GetBranchAndUrl(branch, manifestUrl)) {
		config_set_string(GetGlobalConfig(), "General", "UpdateBranch", WIN_DEFAULT_BRANCH);
	...
	const char *skipUpdateVer = config_get_string(GetGlobalConfig(), "General", "SkipUpdateVersion");
	...
	if (repairMode) {
		...
		if (queryResult == OBSUpdate::No) {
			...
			config_set_string(GetGlobalConfig(), "General", "SkipUpdateVersion", updateVer.c_str());
			return;
		}
	}
	...
	config_set_string(GetGlobalConfig(), "General", "SkipUpdateVersion", "0");
