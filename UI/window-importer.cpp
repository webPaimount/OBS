...
OBSImporter::OBSImporter(QWidget *parent) : QDialog(parent), optionsModel(new ImporterModel), ui(new Ui::OBSImporter)
{
	...
	bool autoSearchPrompt = config_get_bool(App()->GlobalConfig(), "General", "AutoSearchPrompt");
	...
	if (!autoSearchPrompt) {
		...
		if (button == QMessageBox::Yes) {
			config_set_bool(App()->GlobalConfig(), "General", "AutomaticCollectionSearch", true);
		} else {
			config_set_bool(App()->GlobalConfig(), "General", "AutomaticCollectionSearch", false);
		}

		config_set_bool(App()->GlobalConfig(), "General", "AutoSearchPrompt", true);
	}

	bool autoSearch = config_get_bool(App()->GlobalConfig(), "General", "AutomaticCollectionSearch");
...
void OBSImporter::importCollections()
{
	...
	for (int i = 0; i < optionsModel->rowCount() - 1; i++) {
		...
		if (res != json11::Json()) {
			...
			blog(LOG_INFO, "Import Scene Collection: %s (%s) - %s", name.c_str(), file.c_str(),
