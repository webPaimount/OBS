...
#undef ADD_HOTKEY_FOCUS_TYPE
	...
	connect(ui->simpleReplayBuf, &QCheckBox::toggled, this, &OBSBasicSettings::SimpleReplayBufferChanged);
...
void OBSBasicSettings::LoadThemeList()
{
	...
	int ret = GetConfigPath(userThemeDir, sizeof(userThemeDir), "obs-studio/themes/");
...
void OBSBasicSettings::LoadBranchesList()
{
	...
	QString configBranch = config_get_string(GetGlobalConfig(), "General", "UpdateBranch");
...
void OBSBasicSettings::LoadGeneralSettings()
{
	...
	if (ui->hideOBSFromCapture) {
		bool hideWindowFromCapture =
			config_get_bool(GetGlobalConfig(), "BasicWindow", "HideOBSWindowsFromCapture");
	...
	bool spacingHelpersEnabled = config_get_bool(GetGlobalConfig(), "BasicWindow", "SpacingHelpersEnabled");
	...
	bool safeAreas = config_get_bool(GetGlobalConfig(), "BasicWindow", "ShowSafeAreas");
	...
	ui->multiviewLayout->setCurrentIndex(ui->multiviewLayout->findData(
		QVariant::fromValue(config_get_int(GetGlobalConfig(), "BasicWindow", "MultiviewLayout"))));
...
void OBSBasicSettings::LoadAudioSettings()
{
	...
	bool enableLLAudioBuffering = config_get_bool(GetGlobalConfig(), "Audio", "LowLatencyAudioBuffering");
...
void OBSBasicSettings::SaveGeneralSettings()
{
	...
	if (WidgetChanged(ui->theme)) {
		...
		config_set_string(GetGlobalConfig(), "General", "CurrentTheme3", QT_TO_UTF8(themeData));
	}
	...
	if (WidgetChanged(ui->updateChannelBox)) {
		config_set_string(GetGlobalConfig(), "General", "UpdateBranch", QT_TO_UTF8(branchName));
	...
	if (ui->hideOBSFromCapture && WidgetChanged(ui->hideOBSFromCapture)) {
		...
		config_set_bool(GetGlobalConfig(), "BasicWindow", "HideOBSWindowsFromCapture", hide_window);
	...
	config_set_bool(GetGlobalConfig(), "BasicWindow", "WarnBeforeStoppingRecord",
	...
	if (WidgetChanged(ui->projectorAlwaysOnTop)) {
		config_set_bool(GetGlobalConfig(), "BasicWindow", "ProjectorAlwaysOnTop",
	...
	if (WidgetChanged(ui->recordWhenStreaming))
		config_set_bool(GetGlobalConfig(), "BasicWindow", "RecordWhenStreaming",
	...
	if (WidgetChanged(ui->replayWhileStreaming))
		config_set_bool(GetGlobalConfig(), "BasicWindow", "ReplayBufferWhileStreaming",
				ui->replayWhileStreaming->isChecked());
	if (WidgetChanged(ui->keepReplayStreamStops))
		config_set_bool(GetGlobalConfig(), "BasicWindow", "KeepReplayBufferStreamStops",
	...
	if (WidgetChanged(ui->systemTrayAlways))
		config_set_bool(GetGlobalConfig(), "BasicWindow", "SysTrayMinimizeToTray",
	...
	if (WidgetChanged(ui->closeProjectors))
		config_set_bool(GetGlobalConfig(), "BasicWindow", "CloseExistingProjectors",
				ui->closeProjectors->isChecked());

	if (WidgetChanged(ui->studioPortraitLayout)) {
		config_set_bool(GetGlobalConfig(), "BasicWindow", "StudioPortraitLayout",
	...
	if (WidgetChanged(ui->prevProgLabelToggle)) {
		config_set_bool(GetGlobalConfig(), "BasicWindow", "StudioModeLabels",
	...
	if (WidgetChanged(ui->multiviewMouseSwitch)) {
		config_set_bool(GetGlobalConfig(), "BasicWindow", "MultiviewMouseSwitch",
				ui->multiviewMouseSwitch->isChecked());
		multiviewChanged = true;
	}

	if (WidgetChanged(ui->multiviewDrawNames)) {
		config_set_bool(GetGlobalConfig(), "BasicWindow", "MultiviewDrawNames",
				ui->multiviewDrawNames->isChecked());
		multiviewChanged = true;
	}

	if (WidgetChanged(ui->multiviewDrawAreas)) {
		config_set_bool(GetGlobalConfig(), "BasicWindow", "MultiviewDrawAreas",
				ui->multiviewDrawAreas->isChecked());
		multiviewChanged = true;
	}

	if (WidgetChanged(ui->multiviewLayout)) {
		config_set_int(GetGlobalConfig(), "BasicWindow", "MultiviewLayout",
...
void OBSBasicSettings::SaveAdvancedSettings()
{
	...
	if (WidgetChanged(ui->confirmOnExit))
		config_set_bool(GetGlobalConfig(), "General", "ConfirmOnExit", ui->confirmOnExit->isChecked());
...
void OBSBasicSettings::SaveAudioSettings()
{
	...
	if (WidgetChanged(ui->lowLatencyBuffering)) {
		...
		config_set_bool(GetGlobalConfig(), "Audio", "LowLatencyAudioBuffering", enableLLAudioBuffering);
	}
...
void OBSBasicSettings::on_advOutFFAEncoder_currentIndexChanged(int idx)
{
	...
	if (!itemDataVariant.isNull()) {
		...
		SetAdvOutputFFmpegEnablement(AUDIO, desc.id != 0 || desc.name != nullptr, true);
	}
}
...
void OBSBasicSettings::on_advOutFFVEncoder_currentIndexChanged(int idx)
{
	...
	if (!itemDataVariant.isNull()) {
		auto desc = itemDataVariant.value<FFmpegCodec>();
		SetAdvOutputFFmpegEnablement(VIDEO, desc.id != 0 || desc.name != nullptr, true);
	}
}
...
void OBSBasicSettings::HideOBSWindowWarning(int state)
{
	...
	if (config_get_bool(GetGlobalConfig(), "General", "WarnedAboutHideOBSFromCapture"))
		return;
	...
	config_set_bool(GetGlobalConfig(), "General", "WarnedAboutHideOBSFromCapture", true);
	config_save_safe(GetGlobalConfig(), "tmp", nullptr);
}
...
void OBSBasicSettings::AdvOutRecCheckWarnings()
{
	...
	if (!errorMsg.isEmpty() || !warningMsg.isEmpty()) {
		...
		advOutRecWarning->setObjectName(errorMsg.isEmpty() ? "warningLabel" : "errorLabel");
		advOutRecWarning->setWordWrap(true);

		QFormLayout *formLayout = reinterpret_cast<QFormLayout *>(ui->advOutRecTopContainer->layout());
...
void OBSBasicSettings::UpdateAutomaticReplayBufferCheckboxes()
{
	...
	case 0:
		...
		ui->simpleReplayBuf->setEnabled(!obs_frontend_replay_buffer_active());
		break;
	case 1:
