...
#undef SETUP_DOCK
	...
	connect(ui->broadcastButton, &QPushButton::clicked, this, &OBSBasic::BroadcastButtonClicked);
...
bool OBSBasic::LoadService()
{
	...
	if (strcmp(obs_service_get_protocol(service), "FTL") == 0 ||
		...
		const char *option = config_get_string(basicConfig, "SimpleOutput", "StreamAudioEncoder");
		if (strcmp(option, "opus") != 0)
			config_set_string(basicConfig, "SimpleOutput", "StreamAudioEncoder", "opus");
...
bool OBSBasic::InitBasicConfigDefaults()
{
	...
	auto MigrateFormat = [&](const char *section) {
		...
		string old_format = config_get_string(basicConfig, section, has_new_key ? "RecFormat2" : "RecFormat");
...
{
	vcamButton = new ControlsSplitButton(QTStr("Basic.Main.StartVirtualCam"), "vcamButton",
					     &OBSBasic::VCamButtonClicked);
	vcamButton->addIcon(QTStr("Basic.Main.VirtualCamConfig"), QStringLiteral("configIconSmall"),
...
void OBSBasic::ResetOutputs()
{
	...
	if (!outputHandler || !outputHandler->Active()) {
		...
		if (outputHandler->replayBuffer) {
			replayBufferButton = new ControlsSplitButton(QTStr("Basic.Main.StartReplayBuffer"),
								     "replayBufferButton",
...
void OBSBasic::OBSInit()
{
	...
	ret = snprintf(fileName, sizeof(fileName), "obs-studio/basic/scenes/%s.json", sceneCollection);
	if (ret <= 0)
...
#undef SET_VISIBILITY
	...
	bool sysTrayEnabled = config_get_bool(App()->GlobalConfig(), "BasicWindow", "SysTrayEnabled");
	bool sysTrayWhenStarted = config_get_bool(App()->GlobalConfig(), "BasicWindow", "SysTrayWhenStarted");
	...
	bool sideDocks = config_get_bool(App()->GlobalConfig(), "BasicWindow", "SideDocks");
...
void OBSBasic::ReceivedIntroJson(const QString &text)
{
	...
	constexpr uint64_t currentVersion = (uint64_t)LIBOBS_API_VER << 16ULL | OBS_RELEASE_CANDIDATE << 8ULL |
					    OBS_BETA;
	uint64_t lastVersion = config_get_uint(App()->GlobalConfig(), "General", lastInfoVersion);
	...
	if ((lastVersion & ~0xFFFF0000ULL) < (currentVersion & ~0xFFFF0000ULL)) {
		...
		config_set_uint(App()->GlobalConfig(), "General", lastInfoVersion, currentVersion);
	} else {
		current_version_increment = config_get_int(App()->GlobalConfig(), "General", "InfoIncrement");
	}
	...
	config_set_int(App()->GlobalConfig(), "General", "InfoIncrement", info_increment);
	config_save_safe(App()->GlobalConfig(), "tmp", nullptr);
...
void OBSBasic::ShowWhatsNew(const QString &url)
{
	...
	connect(cefWidget, SIGNAL(titleChanged(const QString &)), dlg, SLOT(setWindowTitle(const QString &)));
...
void OBSBasic::CreateHotkeys()
{
	...
	auto LoadHotkeyPair = [&](obs_hotkey_pair_id id, const char *name0, const char *name1,
		...
		if (oldName) {
			const auto info = config_get_string(basicConfig, "Hotkeys", oldName);
			if (info) {
				config_set_string(basicConfig, "Hotkeys", name0, info);
				config_set_string(basicConfig, "Hotkeys", name1, info);
				config_remove_value(basicConfig, "Hotkeys", oldName);
...
#define MAKE_CALLBACK(pred, method, log_action)                            \
	...
	streamingHotkeys = obs_hotkey_pair_register_frontend(
		...
		MAKE_CALLBACK(!basic.outputHandler->StreamingActive() && basic.ui->streamButton->isEnabled(),
	...
	recordingHotkeys = obs_hotkey_pair_register_frontend(
		...
		MAKE_CALLBACK(!basic.outputHandler->RecordingActive() && !basic.ui->recordButton->isChecked(),
	...
	pauseHotkeys = obs_hotkey_pair_register_frontend(
		"OBSBasic.PauseRecording", Str("Basic.Main.PauseRecording"), "OBSBasic.UnpauseRecording",
		Str("Basic.Main.UnpauseRecording"),
		MAKE_CALLBACK(basic.pause && !basic.pause->isChecked(), basic.PauseRecording, "Pausing recording"),
		MAKE_CALLBACK(basic.pause && basic.pause->isChecked(), basic.UnpauseRecording, "Unpausing recording"),
		this, this);
...
OBSBasic::~OBSBasic()
{
	...
	config_set_bool(App()->GlobalConfig(), "BasicWindow", "AlwaysOnTop", ui->actionAlwaysOnTop->isChecked());
	...
	config_set_bool(App()->GlobalConfig(), "BasicWindow", "DocksLocked", ui->lockDocks->isChecked());
	config_set_bool(App()->GlobalConfig(), "BasicWindow", "SideDocks", ui->sideDocks->isChecked());
	config_save_safe(App()->GlobalConfig(), "tmp", nullptr);
...
void OBSBasic::HideAudioControl()
{
	...
	if (!SourceMixerHidden(source)) {
		...
		* with newer versions of QT. */
		QMetaObject::invokeMethod(this, "DeactivateAudioSource", Qt::QueuedConnection,
...
int OBSBasic::ResetVideo()
{
	...
	if (ret == OBS_VIDEO_SUCCESS) {
		...
		const float sdr_white_level = (float)config_get_uint(basicConfig, "Video", "SdrWhiteLevel");
...
bool OBSBasic::ResetAudio()
{
	...
	bool lowLatencyAudioBuffering = config_get_bool(GetGlobalConfig(), "Audio", "LowLatencyAudioBuffering");
...
void OBSBasic::on_actionShow_Recordings_triggered()
{
	...
	const char *adv_path = strcmp(type, "Standard") ? config_get_string(basicConfig, "AdvOut", "FFFilePath")
							: config_get_string(basicConfig, "AdvOut", "RecFilePath");
	const char *path = strcmp(mode, "Advanced") ? config_get_string(basicConfig, "SimpleOutput", "FilePath")
						    : adv_path;
...
void OBSBasic::on_actionAdvAudioProperties_triggered()
{
	...
	bool iconsVisible = config_get_bool(App()->GlobalConfig(), "BasicWindow", "ShowSourceIcons");
...
void OBSBasic::GridActionClicked()
{
	...
	config_set_bool(App()->GlobalConfig(), "BasicWindow", "gridMode", gridMode);
}
...
void OBSBasic::on_scenes_itemDoubleClicked(QListWidgetItem *witem)
{
	...
	if (IsPreviewProgramMode()) {
		bool doubleClickSwitch =
			config_get_bool(App()->GlobalConfig(), "BasicWindow", "TransitionOnDoubleClick");
...
void OBSBasic::YoutubeStreamCheck(const std::string &key)
{
	...
	while (StreamingActive()) {
		...
		if (status == "active") {
			QMetaObject::invokeMethod(ui->broadcastButton, "setEnabled", Q_ARG(bool, true));
			break;
		} else {
			QThread::sleep(1);
			timeout++;
...
void OBSBasic::ShowYouTubeAutoStartWarning()
{
	auto msgBox = []() {
		...
		if (cb->isChecked()) {
			config_set_bool(App()->GlobalConfig(), "General", "WarnedAboutYouTubeAutoStart", true);
			config_save_safe(App()->GlobalConfig(), "tmp", nullptr);
		}
	};

	bool warned = config_get_bool(App()->GlobalConfig(), "General", "WarnedAboutYouTubeAutoStart");
	if (!warned) {
...
void OBSBasic::StartStreaming()
{
	...
	if (!autoStartBroadcast) {
		...
		if (!autoStopBroadcast) {
			...
			ui->broadcastButton->setText(QTStr("Basic.Main.AutoStopEnabled"));
			ui->broadcastButton->setEnabled(false);
		}
	...
	bool replayBufferWhileStreaming =
		config_get_bool(GetGlobalConfig(), "BasicWindow", "ReplayBufferWhileStreaming");
...
void OBSBasic::AutoRemux(QString input, bool no_show)
{
	...
	const obs_encoder_t *audioEncoder = obs_output_get_audio_encoder(outputHandler->fileOutput, 0);
...
void OBSBasic::ReplayBufferStopping()
{
	...
	if (sysTrayReplayBuffer)
		sysTrayReplayBuffer->setText(replayBufferButton->first()->text());
...
void OBSBasic::UpdateEditMenu()
{
	...
	ui->actionPasteTransform->setEnabled(hasCopiedTransform && videoCount > 0);
...
OBSProjector *OBSBasic::OpenProjector(obs_source_t *source, int monitor, ProjectorType type)
{
	...
	bool closeProjectors = config_get_bool(GetGlobalConfig(), "BasicWindow", "CloseExistingProjectors");
...
void OBSBasic::on_resetDocks_triggered(bool force)
{
	...
	{
		QMessageBox::StandardButton button =
			QMessageBox::question(this, QTStr("ResetUIWarning.Title"), QTStr("ResetUIWarning.Text"));
...
#undef RESET_DOCKLIST
	...
	QList<QDockWidget *> docks{ui->scenesDock, ui->sourcesDock, ui->mixerDock, ui->transitionsDock,
				   ui->controlsDock};
...
{
	config_set_bool(App()->GlobalConfig(), "BasicWindow", "ShowContextToolbars", visible);
...
void OBSBasic::on_toggleSourceIcons_toggled(bool visible)
{
	...
	config_set_bool(App()->GlobalConfig(), "BasicWindow", "ShowSourceIcons", visible);
}
...
void OBSBasic::SystemTrayInit()
{
	...
	connect(sysTrayStream, &QAction::triggered, this, &OBSBasic::on_streamButton_clicked);
	connect(sysTrayRecord, &QAction::triggered, this, &OBSBasic::on_recordButton_clicked);
...
{
	if (!pause || !outputHandler || !outputHandler->fileOutput || os_atomic_load_bool(&recording_paused))
		return;
...
{
	if (!activate || !outputHandler || !outputHandler->ReplayBufferActive()) {
		replayBufferButton->removeIcon();
		return;
	}

	replayBufferButton->addIcon(QTStr("Basic.Main.SaveReplay"), QStringLiteral("replayIconSmall"),
...
void OBSBasic::UpdatePreviewOverflowSettings()
{
	...
	bool select = config_get_bool(App()->GlobalConfig(), "BasicWindow", "OverflowSelectionHidden");
	bool always = config_get_bool(App()->GlobalConfig(), "BasicWindow", "OverflowAlwaysVisible");
...
void OBSBasic::SetDisplayAffinity(QWindow *window)
{
	...
	bool hideFromCapture = config_get_bool(App()->GlobalConfig(), "BasicWindow", "HideOBSWindowsFromCapture");
...
QColor OBSBasic::GetSelectionColor() const
{
	if (config_get_bool(GetGlobalConfig(), "Accessibility", "OverrideColors")) {
		return color_from_int(config_get_int(GetGlobalConfig(), "Accessibility", "SelectRed"));
	} else {
...
{
	if (config_get_bool(GetGlobalConfig(), "Accessibility", "OverrideColors")) {
		return color_from_int(config_get_int(GetGlobalConfig(), "Accessibility", "SelectGreen"));
	} else {
...
{
	if (config_get_bool(GetGlobalConfig(), "Accessibility", "OverrideColors")) {
		return color_from_int(config_get_int(GetGlobalConfig(), "Accessibility", "SelectBlue"));
	} else {
...
{
	drawSpacingHelpers = config_get_bool(App()->GlobalConfig(), "BasicWindow", "SpacingHelpersEnabled");
}
