...
void Multiview::Render(uint32_t cx, uint32_t cy)
{
	...
	for (size_t i = 0; i < maxSrcs; i++) {
		...
		gs_matrix_push();
		gs_matrix_translate3f(sourceX + offset, (scenesCY * 0.85f) + sourceY, 0.0f);
		gs_matrix_scale3f(ppiScaleX, ppiScaleY, 1.0f);
		drawBox(obs_source_get_width(label), obs_source_get_height(label) + int(sourceY * 0.015f), labelColor);
	...
	if (drawLabel) {
		...
		drawBox(obs_source_get_width(previewLabel),
			obs_source_get_height(previewLabel) + int(pvwprgCX * 0.015f), labelColor);
	...
	if (drawLabel) {
		...
		drawBox(obs_source_get_width(programLabel),
			obs_source_get_height(programLabel) + int(pvwprgCX * 0.015f), labelColor);
