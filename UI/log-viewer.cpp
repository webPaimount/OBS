...
OBSLogViewer::OBSLogViewer(QWidget *parent) : QDialog(parent), ui(new Ui::OBSLogViewer)
{
	...
	const char *geom = config_get_string(App()->GlobalConfig(), "LogViewer", "geometry");
...
{
	config_set_string(App()->GlobalConfig(), "LogViewer", "geometry", saveGeometry().toBase64().constData());
}

void OBSLogViewer::on_showStartup_clicked(bool checked)
{
	config_set_bool(App()->GlobalConfig(), "LogViewer", "ShowLogStartup", checked);
}
