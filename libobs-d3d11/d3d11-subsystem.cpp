...
void gs_device::InitCompiler()
{
	...
	while (ver > 30) {
		snprintf(d3dcompiler, sizeof(d3dcompiler), "D3DCompiler_%02d.dll", ver);

		HMODULE module = LoadLibraryA(d3dcompiler);
		if (module) {
			d3dCompile = (pD3DCompile)GetProcAddress(module, "D3DCompile");
			d3dCreateBlob = (pD3DCreateBlob)GetProcAddress(module, "D3DCreateBlob");

#ifdef DISASSEMBLE_SHADERS
			d3dDisassemble = (pD3DDisassemble)GetProcAddress(module, "D3DDisassemble");
#endif
...
bool device_enum_adapters(bool (*callback)(void *param, const char *name, uint32_t id), void *param)
{
	try {
...
static inline void LogAdapterMonitors(IDXGIAdapter1 *adapter)
{
	...
	for (i = 0; adapter->EnumOutputs(i, &output) == S_OK; ++i) {
		...
		blog(LOG_INFO,
		     ...
		     desc.AttachedToDesktop ? "true" : "false", refresh, bits_per_color, space, nits, min_luminance,
