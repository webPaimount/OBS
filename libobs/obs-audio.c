...
#endif
		for (size_t ch = 0; ch < channels; ch++)
			circlebuf_pop_front(&source->audio_input_buf[ch], NULL, source->audio_input_buf[0].size);
...
#endif
		for (size_t ch = 0; ch < channels; ch++)
			circlebuf_pop_front(&source->audio_input_buf[ch], NULL, drop * sizeof(float));
...
static void set_fixed_audio_buffering(struct obs_core_audio *audio, size_t sample_rate, struct ts_info *ts)
{
	...
	while (ticks--) {
		...
		circlebuf_push_front(&audio->buffered_timestamps, &new_ts, sizeof(new_ts));
	}
