...
static inline bool fill_buffer(struct audio_monitor *monitor)
{
	...
	circlebuf_pop_front(&monitor->new_data, buf->mAudioData, monitor->buffer_size);
...
static bool audio_monitor_init(struct audio_monitor *monitor, obs_source_t *source)
{
	...
	for (size_t i = 0; i < 3; i++) {
		...
		circlebuf_push_back(&monitor->empty_buffers, &monitor->buffers[i], sizeof(monitor->buffers[i]));
	}
