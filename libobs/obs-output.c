...
static void default_raw_audio_callback(void *param, size_t mix_idx, struct audio_data *in)
{
	...
	for (size_t i = 0; i < output->planes; i++)
		circlebuf_push_back(&output->audio_buffer[mix_idx][i], out.data[i], out.frames * output->audio_size);

	/* -------------- */

	while (output->audio_buffer[mix_idx][0].size > frame_size_bytes) {
		for (size_t i = 0; i < output->planes; i++) {
			circlebuf_pop_front(&output->audio_buffer[mix_idx][i], output->audio_data[i], frame_size_bytes);
...
static inline void pair_encoders(obs_output_t *output)
{
	...
	if (video && audio) {
		...
		if (!audio->active && !video->active && !video->paired_encoder && !audio->paired_encoder) {
...
void obs_output_output_caption_text2(obs_output_t *output, const char *text, double display_duration)
{
	...
	output->caption_tail =
		caption_text_new(text, size, output->caption_tail, &output->caption_head, display_duration);
