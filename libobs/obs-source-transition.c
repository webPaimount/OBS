...
static inline bool activate_child(obs_source_t *transition, size_t idx)
{
	...
	if (transition->transition_sources[idx] && !transition->transition_source_active[idx]) {

		success = obs_source_add_active_child(transition, transition->transition_sources[idx]);
		if (success)
...
static inline void enum_child(obs_source_t *tr, obs_source_t *child, obs_source_enum_proc_t enum_callback, void *param)
{
	if (!child)
		return;

	if (child->context.data && child->info.enum_active_sources)
		child->info.enum_active_sources(child->context.data, enum_callback, param);
