				  ...
				  uint32_t *cy)
{
	...
	if (bounds_type == OBS_BOUNDS_SCALE_INNER || bounds_type == OBS_BOUNDS_SCALE_OUTER) {
		...
		mul = use_width ? item->bounds.x / width : item->bounds.y / height;
...
static void update_item_transform(struct obs_scene_item *item, bool update_tex)
{
	...
	matrix4_translate3f(&item->draw_transform, &item->draw_transform, item->pos.x, item->pos.y, 0.0f);
	...
	matrix4_translate3f(&item->box_transform, &item->box_transform, item->pos.x, item->pos.y, 0.0f);
						...
						obs_sceneitem_t *group_sceneitem)
{
	...
	while (item) {
		...
		if (item->is_group) {
			...
			update_transforms_and_prune_sources(group_scene, remove_items, item);
			video_unlock(group_scene);
		}

		if (os_atomic_load_bool(&item->update_transform) || source_size_changed(item)) {
