...
static inline bool queue_frame(struct obs_core_video_mix *video, bool raw_active, struct obs_vframe_info *vframe_info)
{
	...
	if (duplicate) {
		struct obs_tex_frame *tf =
			circlebuf_data(&video->gpu_encoder_queue, video->gpu_encoder_queue.size - sizeof(*tf));
...
static void output_gpu_encoders(struct obs_core_video_mix *video, bool raw_active)
{
	...
	circlebuf_pop_front(&video->vframe_info_buffer_gpu, &vframe_info, sizeof(vframe_info));
...
static inline void video_sleep(struct obs_core_video *video, uint64_t *p_time, uint64_t interval_ns)
{
	...
	for (size_t i = 0, num = obs->video.mixes.num; i < num; i++) {
		...
		if (raw_active)
			circlebuf_push_back(&video->vframe_info_buffer, &vframe_info, sizeof(vframe_info));
		if (gpu_active)
			circlebuf_push_back(&video->vframe_info_buffer_gpu, &vframe_info, sizeof(vframe_info));
	}
